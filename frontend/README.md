# Vue-Webpack-JSTodoApp

## 关于项目

这是一个为了学习vue和webpack实现的一个todo-list demo，组件中客串了React。
...
npm install
```
2. （可选操作）通过cnpm命令, 需要安装淘宝镜像 http://npm.taobao.org/ 
```sh
cnpm install
```
3. yarn （可选操作，推荐使用yarn,需要先安装yarn https://yarnpkg.com/lang/zh-hans/docs/install/#windows-stable)
```sh
yarn
```
...
我读过一本书《Vue权威指南》，书头提到Vue.js不是一个框架而是一个库，这是一大误区。
